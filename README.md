...
AI-powered tool by SCIKIQ to analyze and convert REST APIs to Anthropic MCP (Model Context Protocol) tools.
...
3. **Scan your project**
   - Click "Scan Project"
   - Select the Python file containing your API routes
...
5. **Convert to MCP**
   ...
   - Follow setup instructions to integrate with Claude Desktop
...
├── intelligent_code_analyzer.py    # Code analysis utilities
├── mcp_converter.py                # MCP conversion logic
├── fastapi_to_mcp_converter.py     # FastAPI-specific converter
├── templates/
...
1. **Scanning Phase**
   - Parses Python files to detect API routes
   - Uses AST (Abstract Syntax Tree) analysis
   - Identifies Flask, FastAPI, Django endpoints

2. **Analysis Phase**
   - Sends endpoint code to Azure OpenAI GPT-4
...
4. **Integration Phase**
   ...
   - Shows how to configure Claude Desktop
   - Offers code preview with download
